...
function getSecret() {
  ...
  if (!secret) {
    throw new Error("AUTH_SECRET is not set. Add it to .env before starting the server.");
  }
  return secret;
}
...
// Wraps a Next.js API route handler so it 401s unless a valid session cookie is present.
