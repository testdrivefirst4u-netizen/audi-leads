...
class MetaApiError extends Error {
  ...
  get isAuthError() {
    return this.type === "OAuthException" || this.code === 190 || this.code === 102;
  }
...
async function getForm(formId, token) {
  return graphRequest(formId, { token, params: { fields: "id,name,status,locale,questions{key,label,type}" } });
}

async function getPage(pageId, token) {
  return graphRequest(pageId, { token, params: { fields: "id,name,instagram_business_account{id,username}" } });
}
