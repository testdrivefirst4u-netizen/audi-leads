...
function resolveModel(lead, data, formName) {
  const explicit = firstValue(lead.field_data, [/^(vehicle_)?model$/i, /model/i, /vehicle/i, /^car$/i, /interested_in/i]);
  const raw = explicit || formName || lead.campaign_name || "Meta Lead Ads";
  const canonical = canonicalModelFor(raw);
  return { model: raw, canonicalModel: canonical === "Other" ? raw : canonical };
}
...
module.exports = { mapMetaLeadToCrm, fieldDataToObject, platformOf };
