...
const { mapMetaLeadToCrm, isMetaTestLead } = require("./mapLead");
...
async function processEvent(event) {
  if (event.status === "processed" || event.status === "duplicate" || event.status === "test") return event;
  ...
  try {
    ...
    if (isMetaTestLead(lead)) {
      event.status = "test";
      event.platform = lead.platform === "ig" ? "instagram" : "facebook";
      event.lastError = "";
      event.processedAt = new Date();
      await event.save();
