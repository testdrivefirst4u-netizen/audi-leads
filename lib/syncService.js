...
const { canonicalModelFor, parseSheetDate, FIELD_MATCHERS } = require("./leadFields");
...
// re-querying Mongo for counts on every row.
async function createAgentAssigner() {
  ...
  return function assignNext() {
    ...
    countMap.set(leastId, leastCount + 1);
    return leastId;
  };
}
  ...
  try {
    ...
    for (const tab of tabs) {
      ...
      for (const { rowNumber, record: rawRecord } of records) {
        ...
        // Dedupe by sheet row position, not by phone/lead ID: the same person
        // submitting the form again is a genuinely new lead/opportunity and
        // should get its own row in the CRM, not merge into their earlier
        // submission. Row position still makes re-running sync idempotent
        // (the same row won't create a duplicate lead on every tick), since
        // these sheets are append-only exports.
        const query = { model: tab, rowNumber };
