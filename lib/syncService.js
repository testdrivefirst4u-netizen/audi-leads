...
const { canonicalModelFor, parseSheetDate, resolveLocation, applySourceMap, FIELD_MATCHERS } = require("./leadFields");
...
// a lead's showroom is matched against each agent's `locations` first (an
// agent with none covers the general pool, i.e. any showroom); only if no agent
...
async function createAgentAssigner(companyId) {
  ...
  return function assignNext(location) {
    if (location) {
      const local = agents.filter((a) => agentLocations(a).includes(location)).map((a) => String(a._id));
...
// Multi-select `locations` with the legacy single `location` as fallback
// for agents saved before the field existed.
function agentLocations(agent) {
  if (Array.isArray(agent.locations) && agent.locations.length) return agent.locations;
  return agent.location ? [agent.location] : [];
}
...
function isTestLead(record) {
  return Object.values(record).some((v) => typeof v === "string" && /test lead:\s*dummy data/i.test(v));
}

// Pass 1 of the sync, for one raw sheet row: strip Meta's ID prefixes,
// pull out the identity fields, and decide whether the row is usable at
// all. Returns { skipReason } for rows the sync will never ingest, else the
// parsed row. Shared with lib/importAudit.js so "why isn't this row in the
// CRM?" is answered by the exact same rules that decided not to import it —
// any change to what the sync skips must be made here, once.
const SKIP_REASONS = {
  test: "Meta test lead (dummy data)",
  noIdentity: "No phone number or lead ID to identify the customer",
};

function parseSheetRow(rawRecord, settings) {
  if (isTestLead(rawRecord)) return { skipReason: SKIP_REASONS.test };
  ...
  if (!leadId && !phone) return { skipReason: SKIP_REASONS.noIdentity, name, email };
  ...
  return {
    ...
    location: resolveLocation(record, settings.locationField),
  };
}
...
// Ingests one tab's rows — the whole sync decision procedure for a tab:
// parse (parseSheetRow), decide idempotently against what is already in
// Mongo (same tab+rowNumber → update/unchanged; already folded into an
// enquiryHistory → unchanged; already created by the Meta webhook for the
// same leadgen id → link), then dedupeAndCreateLead for what is genuinely
// new. runSync() calls this once per tab with the full tab; the instant
// push endpoint (pages/api/public/sheet-rows.js) calls it with just the
// rows that were appended, so both paths can never disagree.
async function ingestTabRows({ companyId, settings, tab, records, assignNext }) {
  const counts = {
  ...
  const sheetLeadIds = parsedRows.map((r) => r.leadId).filter(Boolean);
  const [existingDocs, recordedElsewhereDocs, webhookDocs] = await Promise.all([
    ...
    Lead.find({
      ...
      .lean(),
    // Leads the Meta webhook (lib/meta/processEvent.js) already created
    // for these exact leadgen ids — see the note above.
    sheetLeadIds.length
      ? Lead.find({ companyId, metaLeadId: { $in: sheetLeadIds } })
          .select("metaLeadId rowNumber model")
          .lean()
      : Promise.resolve([]),
  ]);
  const webhookLeadByMetaId = new Map(webhookDocs.map((d) => [d.metaLeadId, d]));
  ...
  for (const row of parsedRows) {
    ...
    // Same Meta lead, delivered by the webhook before this sheet row
    // was synced: link the row to that lead (so future runs see it as
    // this row's primary lead and the Import Audit reconciles) instead
    // of creating a duplicate or a bogus repeat enquiry.
    const viaWebhook = row.leadId && webhookLeadByMetaId.get(row.leadId);
    if (viaWebhook && !viaWebhook.rowNumber) {
      await Lead.updateOne(
        { _id: viaWebhook._id },
        { $set: { model: tab, rowNumber: row.rowNumber, contentHash: row.contentHash, leadId: row.leadId } },
      );
      counts.updatedCount++;
      continue;
    }
    ...
    const { status } = await dedupeAndCreateLead({
      ...
      canonicalModel,
      data: row.record,
      source: applySourceMap("Meta Ads", settings.sourceMap),
    ...
    if (status === "duplicate") counts.duplicateCount++;
  ...
  const lockAcquired = await Settings.findOneAndUpdate(
    {
      companyId,
      $or: [
        { "syncLock.inProgress": { $ne: true } },
        { "syncLock.startedAt": { $lt: new Date(Date.now() - SYNC_LOCK_STALE_MS) } },
      ],
    },
  ...
  try {
    ...
    for (const sheetSource of settings.sheets) {
      ...
      for (const tab of tabs) {
        ...
        const c = await ingestTabRows({ companyId, settings, tab, records, assignNext });
...
async function buildStatusPayload(latestLog, companyId) {
  ...
  const online =
    !!latestSuccess && Date.now() - new Date(latestSuccess.finishedAt || latestSuccess.startedAt).getTime() < thresholdMs;
